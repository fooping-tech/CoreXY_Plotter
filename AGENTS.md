...
デバッグ中、実機bring-up中、または不具合対策中に、当初計画にない追加仕様・制約・暫定運用・安全対策を入れた場合は、コード変更だけで終わらせない。
...
- 脱調対策として速度、加速度、TMC電流、ペン角度を変更した場合
- limit入力ノイズ対策やhard limit判定の条件を追加した場合
- 実機テスト用CSVやチェック手順を追加した場合
- 一時的な安全側設定や、後で再調整が必要な暫定値を導入した場合

未確認の温度、停止距離、配線ノイズ、連続実行耐性などは、完了扱いにせず`PLANS.md`のリスクまたは未解決事項として残すこと。
...
デバイス未接続、upload port不明、実機操作に危険がある等の理由で書き込みまたは
動作テストを実行できない場合は、未実行の手順と理由を明確に報告する。
