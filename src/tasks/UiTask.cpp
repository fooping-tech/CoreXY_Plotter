...
void drawControlPage(const StatusMessage& status) {
  ...
  drawButton({119, 124, 82, 34}, "+1mm", COLOR_ACCENT, enabled, 1);
...
void queueJog(const StatusMessage& status, float dx_mm, float dy_mm) {
  ...
  command.x_mm = clampFloat(state.x_mm + dx_mm, X_MIN_MM, X_MAX_MM);
  command.y_mm = clampFloat(state.y_mm + dy_mm, Y_MIN_MM, Y_MAX_MM);
  command.feed_mm_min = UI_JOG_FEED_MM_MIN;
  if (fabsf(command.x_mm - state.x_mm) < 0.001f &&
      fabsf(command.y_mm - state.y_mm) < 0.001f) {
    setNotice("Soft limit");
    return;
  }
  if (queueCommand(command)) setNotice("Jog queued");
}
